...
- **Iconos**: cada comisión y cada grupo puede elegir un icono de una lista cerrada. Se usa donde no cabe una imagen, como la etiqueta de comisión junto a una persona, y como respaldo de la portada o del logo. El Pleno lleva siempre una estrella.
- **Orden**: las comisiones, los grupos y las opciones de voto se ordenan arrastrando. Los botones de subir y bajar hacen lo mismo sin ratón. El Pleno no es una comisión y no se coloca entre ellas: se arrastra sobre la tabla y va antes o después de todas.
- **Color de una opción**: si se cambia, aparece un botón para volver al color por defecto de esa posición.
...
- **Programación**: cada votación puede llevar hora de apertura y de cierre, ambas opcionales e independientes. El servidor las revisa cada diez segundos. Los botones de abrir y cerrar mandan siempre: al usarlos, la hora de apertura se da por gastada y la de cierre se descarta si ya había pasado, de modo que abrir a mano nunca se deshace solo. Una apertura programada repite las mismas comprobaciones que el botón, y si la votación está oculta o no tiene opciones válidas se descarta la hora y se registra el motivo. Si la aplicación estuvo parada durante toda la ventana, la votación no se abre tarde y sin nadie delante. "Borrar votos" limpia también las horas.
