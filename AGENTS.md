...
- The plenary has no `committees` row (its id is null throughout the API), so its cover lives in `app_settings` under `plenaryCover` and is served through the committee listing and detail.
