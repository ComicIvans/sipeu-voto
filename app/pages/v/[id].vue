...
const { data, error, refresh } = await useFetch<{ data: VoteDetail }>(`/api/votes/${voteId}`)
...
const showVotingPanel = computed(() => Boolean(user.value) && vote.value.canVote && vote.value.open)
  ...
  <UContainer class="animate-fade-slide-up py-8 sm:py-12">
    ...
    <div class="mt-3 flex flex-wrap items-start justify-between gap-4">
      <div class="min-w-0">
        ...
        <h1 class="text-highlighted mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
          {{ vote.name }}
        </h1>
        ...
        <p class="text-muted mt-2 text-sm">
          <template v-if="vote.open && vote.startedAt">
            Abierta desde {{ formatDateTime(vote.startedAt) }}
          </template>
